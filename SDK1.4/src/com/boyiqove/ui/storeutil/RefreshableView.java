...
public class RefreshableView extends LinearLayout implements OnTouchListener{
	 ...
	 * 需要去下拉刷新的GridView
	 */
	private ShelfGridView listView;
	...
	public RefreshableView(Context context, AttributeSet attrs) {
		...
		header = LayoutInflater.from(context).inflate(R.layout.pull_to_refresh,
				null, true);
		progressBar =(ImageView) header.findViewById(R.id.progress_bar);
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed && !loadOnce) {
			...
			listView = (ShelfGridView) getChildAt(1);
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		if (ableToPull) {
			...
			if (currentStatus == STATUS_PULL_TO_REFRESH
				...
				//防止listview在下拉的时候被短点击
				listView.setPressed(false);
				listView.setFocusable(false);
				listView.setFocusableInTouchMode(false);
				listView.setLongClickable(false);
				//防止listview在下拉的时候被长点击
	...
	private void setIsAbleToPull(MotionEvent event) {
		...
		if (firstChild != null) {
			int firstVisiblePos =firstChild.getTop()-listView.getPaddingTop();
	...
	private void roateImageView() {
		...
		progressBar.startAnimation(operatingAnim);
	}
	...
	private void refreshUpdatedAtValue() {
		...
		String time=getDateToString(currentTime);
		updateAt.setText("最近更新:"+getDateToString(currentTime));
		DebugLog.e(TAG,getDateToString(currentTime));
